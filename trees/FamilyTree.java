...
public class FamilyTree {
    ...
    private void addLine(String line) throws TreeException {
        ...
        throw a TreeException with a useful message String parent =  ?  ? The  substring  of line that starts at char #0 and ends just before colonIndex.Check the API for   
		...
		if (root == null) {
            ...
            parentNode = root. ?  ?  ?  ?  ? There
               ...
               's a method in Node that searches for a named node. 
 ?  ?  ? If  the  parent  node wasn {

            }'t found, there must have been something wrong in the 
    ...
    TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException {
        ...
        if (node1 == null) {
             ?  ?  ? Throw  a  TreeException  with 
        }
        a useful message TreeNode node2 = root. ? 
                ? 
                        ? // node whose name is name2
        if (node2 == null) {
             ?  ?  ? Throw  TreeException  with  a 
        }
        useful message // Get ancestors of node1 and node2.
